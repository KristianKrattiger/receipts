...
**Amendment (applied 2026-08-31, after Task 7's review).** `LedgerRow` and
`AdmittedRelation` carry `sides: AdmittedSpan[]` -- one span for an unsupported
claim, two for a relation, in `from`-then-`to` order. They do NOT carry
`vendorSide` / `independentSide`. Role-keyed slots silently discarded one
validated span whenever both sides shared a role, and `vendor_site`,
`vendor_docs` and `vendor_pricing` all carry role `vendor_claim` -- so a vendor
contradicting its own docs rendered as half a row. Consumers label each side by
looking its document up in `report.docs` and reading that document's `role`.
`zod` is `^4` (the SDK's zod helper needs `z.toJSONSchema`). Structured outputs
are `client.beta.messages.parse` with `output_format`; the result is on
`.parsed`.
...
- Produces:
  ...
  - `interface AdmittedRelation { proposal: RelationProposal; sides: AdmittedSpan[] }`
...
describe("admit — accepts sound proposals", () => {
  it("admits a well-anchored contradiction and assigns sides by role", () => {
    ...
    expect(a.sides.map((s) => s.docId)).toEqual(["vendor", "status"])
  })
