...
Expected: all tests pass. The CLI that consumes this arrives in Task 12.
...
**Interfaces:**
- Consumes: `chunkAll` (Task 3), `buildIdf`/`tokenize` (Task 5), `selectCandidates` (Task 6), `admit` (Task 7), `proposeRelations`/`ProposalClient` (Task 8), `buildReport` (Task 9), `renderTerminal` (Task 10), `fetchCorpus` (Task 2), `buildSourcePlan` (Task 11).
...
- [ ] **Step 6: Retire the standalone snapshot script**

`src/fetch/snapshot.ts` was a stopgap from Task 2, before `sources/` and the CLI
existed. `--snapshot` now covers it, and leaving both means two hardcoded target
lists that will drift.

```bash
cd receipts && git rm src/fetch/snapshot.ts
```

- [ ] **Step 7: Run the CLI against a fixture**

```bash
cd receipts && npx tsx src/cli/index.ts solari --from-fixture fixtures/solari.json
```

Expected: a rendered ledger with an audit line. First end-to-end run — one Opus 5
call (~$0.14), no browser time. Requires `ANTHROPIC_API_KEY` in the environment.

- [ ] **Step 8: Run it live, end to end**

```bash
cd receipts && npx tsx src/cli/index.ts solari --snapshot fixtures/solari-live.json
```

Expected: a rendered ledger. Costs one browser fan plus one model call, roughly
$0.18. Requires both `SOLARI_API_KEY` and `ANTHROPIC_API_KEY`.

- [ ] **Step 9: Typecheck, then commit**

```bash
cd receipts && npx tsc --noEmit && npx vitest run
```

Expected: no type errors, all tests pass.

```bash
git add -A receipts && git commit -m "feat(receipts): pipeline and CLI with offline fixture replay"
```
...
**Interfaces:**
- Consumes: `fetchCorpus` (Task 2), `analyzeCorpus` (Task 12), `renderMarkdown` (Task 10), `buildSourcePlan` (Task 11).
...
**Interfaces:**
- Consumes: `fetchCorpus` (Task 2), `analyzeCorpus` (Task 12), `renderHtml` (Task 14), `buildSourcePlan` (Task 11).
...
**Spec coverage.** Every module in the spec maps to a task: `sources/` (11), `fetch/` (2), `chunk/` (3), `retrieve/` (5, 6), `cartographer/` (8), `bookkeeper/` (4, 7), `report/` (9, 10, 14). All seven denial codes are implemented in Task 7 and exercised by its tests. All three row statuses are implemented in Task 9. All five invariants are covered: (1) by the property test in Task 7 and the end-to-end check in Task 12; (2) by Task 1's normalization discipline and its idempotence test; (3) by rendering only `span.text` in Task 10 and Task 14; (4) by per-source isolation in Task 2 and the failure rendering tests in Task 10; (5) by the empty-ledger tests in Tasks 9, 10, and 14. Every error-handling requirement has a home: per-source isolation and empty-extraction detection in Task 2, Solari lifecycle in Task 2, refusal and parse failure in Task 8, unknown `docId` in Task 7. All three surfaces ship (12, 13, 14, 15). The 40-word cap is enforced in Task 4 and tested there.
...
**Type consistency.** `AdmitResult` is defined in Task 7 and imported by name in Tasks 9 and 12. `ProposalClient` is defined in Task 8 and injected in Tasks 8 and 12. `buildSourcePlan` is written in Task 11 and imported by the CLI in Task 12, so the CLI never references a module that does not yet exist. `chunkAll`, `selectCandidates`, `buildIdf`, `tokenize`, `admit`, `buildReport`, `renderTerminal`, `renderMarkdown`, `renderHtml`, `fetchCorpus`, and `analyzeCorpus` are each defined once and referenced with matching signatures throughout.

**Ordering.** Risk first (Task 2 answers whether the fan works before anything is built on it), then the offline seam, then the gate before the proposer it gates. Bail-out points: Task 12 is a credible shipped tool on its own; Task 13 makes it agent-native; Task 15 completes the plan.
