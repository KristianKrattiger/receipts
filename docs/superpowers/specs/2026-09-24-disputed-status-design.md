...
- `npm run replay` on receipts' `main`/`reports/tesla-fsd.json`: check whether any of the ledger's existing rows are an unmarked `contradicts`/`updates` (if so, replay will now produce `disputed` instead of `divergent` for that row — a real diff, not a bug — and the ledger needs the same free cache-only regeneration used for the two prior restamps this session). If none are, replay's `1 replayed, 3 not replayable` line is unaffected and no regeneration is needed. Determine which case applies before writing the implementation plan's task list — this changes whether a regeneration task is needed at all.
