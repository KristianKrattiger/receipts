...
- `claim-record/src/instance/calibration/calibration.test.ts`: add one new regression test reproducing the Blue Chip false positive — an unrelated `pin("blue_chip", "independent", "We hold that a private right of action under this provision extends only to actual purchasers or sellers of securities, and standing does not extend to those who merely relied on a misrepresentation without transacting.")` in the pile alongside `STATUTE`, proposing an unmarked `STATUTE`-vs-claim corroboration; assert it is admitted (not denied `HOLDING_COMPETITOR`). (Wording says "this provision," not "Section 10(b)," verified during implementation: literally repeating the statute cite dilutes the corpus-wide IDF weight of "10"/"b" enough to trip the unrelated `NOT_QUERY_RELEVANT` gate instead once a third independent document joins the pile — a real interaction between two admission gates, not a flaw in the redesigned signal itself.)
