...
function ledger(rows: LedgerRow[], over: Partial<Extract<AssayResult, { outcome: "ledger" }>> = {}): AssayResult {
  return {
    ...
    failures: [],
    rows,
    audit: { proposed: rows.length, admitted: rows.length, denied: [], passes: 2, claimantChunks: 0, claimantCovered: 0, claimantOmitted: 0, independentDocsAdmitted: 0 },
    ...over,
  }
}
...
function refusal(): AssayResult {
  return {
    ...
    failures: [],
    nearMiss: [],
    audit: { proposed: 3, admitted: 0, denied: [], passes: 2, claimantChunks: 0, claimantCovered: 0, claimantOmitted: 0, independentDocsAdmitted: 0 },
  }
}
